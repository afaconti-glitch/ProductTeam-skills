...
```bash
claude plugin marketplace add afaconti-glitch/ProductTeam-skills
...
> **Upgrading from v1 or v2?** The layout changed in v3.0.0 — see [Migrating](#migrating-from-v1-or-v2).
...
v3.0.0 moved every skill into a single flat `skills/` directory. If you installed v1 or v2:

```bash
# 1. re-run the installer for your route, then
rm -r ~/.claude/skills/pipeline          # personal installs
rm -r <project>/.claude/skills/pipeline  # project installs
```

v2 nested the pipeline skills one level deeper, so an old install leaves that directory behind and the eight pipeline skills end up present twice. `install.sh` detects this and prints the exact command rather than deleting anything itself.
